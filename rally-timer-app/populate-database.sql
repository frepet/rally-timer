...
-- PostgreSQL schema (dev reference). Timestamps are INTEGER milliseconds since epoch.
-- Note: schema is managed in production by app migrations (000_initial_schema.ts, 001_gates.ts).
...
CREATE INDEX fe_stage_tag_ts_idx ON finish_events(stage_id, tag, timestamp);
...
GROUP BY st.rally_id, st.rally_name, st.driver_id, st.driver_name,
         st.driver_tag, st.class_id, st.class_name;
