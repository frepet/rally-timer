...
export async function DELETE(event: RequestEvent): Promise<Response> {
	...
	const result = db.prepare('DELETE FROM blip_events WHERE id = ?;').run(event.params.id);
	return json(result);
}
...
export async function PATCH({ params, request }: RequestEvent) {
	...
	const row = db
		.prepare(
			`UPDATE blip_events SET timestamp = ? WHERE id = ? RETURNING id, stage_id, timestamp, tag`
		)
		.get(ts, id);
