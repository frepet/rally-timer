...
<div class="mx-auto w-full max-w-5xl space-y-6 p-5">
	...
	{#if tr.gate_id}
			...
			{#if tr.drivers.length}
				<div class="overflow-x-auto">
					<table class="w-full text-sm">
						...
						<tbody>
							{#each tr.drivers as d, i (d.driver_id)}
								<tr
		...
		<!-- Per-driver cards -->
		{#each tr.drivers as d (d.driver_id)}
			{#if d.laps.length}
				<Card class="max-w-none p-4">
					<div class="mb-3 flex items-baseline justify-between gap-2">
						<div>
						...
						{#each d.laps as lap, idx (lap.gate_event_id)}
							...
							>
								<div class="flex items-center gap-3">
									...
									>
									<span class="font-mono {isBest ? 'font-semibold' : ''}"
								...
								{#if $isAdmin}
									<button
										...
										onclick={() => deleteLap(lap.gate_event_id)}
										aria-label={t.delete}
									>
