...
The app currently supports multiple simultaneous rallies in one DB. The owner only runs one rally at a time, wipes the data between events, and will host multiple instances on separate URLs if concurrent rallies are ever needed. Multi-tenancy adds complexity (rally picker UI, junction table, partitioned views, nested API routes) that serves no purpose under this operational model. Removing it simplifies every layer of the stack.
...
6. Recreate all four views without `rally_id` / `rally_name` columns and without `PARTITION BY rally_id`:
   - `stage_times`: same logic, just drop `rally_id` and `rally_name` columns
   - `rally_times`: same aggregation, just group by driver fields (no rally_id grouping)
   - `rally_leaderboard`: ROW_NUMBER OVER (ORDER BY total_ms) with no PARTITION BY rally_id; keep PARTITION BY class_id for class_position
...
Replaces the rally-scoped bundle. Returns:
```json
{ "drivers": [...], "stages": [...], "start_events": [...], "finish_events": [...] }
```
No rally field. Drivers = all drivers in DB (all always active).
...
- `loadAllRaw()` calls `GET /api/bundle` with no ID
...
- Remove: `loadAssigned`, `addToRally`, `removeFromRally`, driver assignment modal
...
- Polling `$effect` simplifies: call `loadStages()` and `loadGates()` only
...
- `drivers` comes from `bundle.drivers` (all drivers — correct, all are always active)
...
4. `/rallies` manage page: stages list loads, create stage works, gates appear, no rally picker visible
5. `/stages/<id>/start` — start gate page loads, all drivers shown, UHF events flow through SSE
6. `/stages/<id>/events` — event list loads, driver names resolve from RFID tags
7. `/` results page — leaderboard renders with no rally dropdown
8. `GET /api/bundle` returns correct shape with no `rally` field
9. UHF gate `uhf_gate.py` — unchanged, posts to `/api/gate-event` which is unchanged
